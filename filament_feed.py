...
class FeedPort:
    def __init__(self, printer, reactor, pin, threshold, index):
        ...
        self.index = index
        try:
            self.ace_device = printer.lookup_object('ace_device')
        except Exception:
    ...
    def get_filament_detected(self):
        if self.ace_device is not None and not self.ace_device.feeder_mode():
            return self.ace_device.get_filament_detect(self.index)
...
class FilamentFeed:
    def __init__(self, config):
        ...
        try:
            self.ace_device = self.printer.lookup_object('ace_device')
        except Exception:
            self.ace_device = None
    ...
    def _check_init_state_timer_handler(self, eventtime):
        ...
        for ch in range(FEED_CHANNEL_NUMS):
            if self._port[ch].get_adc_value() < FEED_PORT_ADC_VAL_MODULE_EXIST or (self.ace_device is not None and not self.ace_device.feeder_mode()):
