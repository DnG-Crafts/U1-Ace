...
import serial, threading, time, logging, json, struct, queue, glob, copy, random, configparser
...
class AceDevice:
    ...
    def _handle_not_printing(self, eventtime):
        ...
        if self._last_active_index != -1 and self.enable_feed_assist:
            for i in range(4):
                self.send_request(
                    request={"method": "stop_feed_assist", "params": {"index": i}},
                    callback=None)
        self._last_active_index = -1
        self._last_active_tool = None
                ...
                if toolhead:
                    ...
                    if active_name != self._last_active_tool:
                        ...
                        if self.enable_feed_assist:
                            self.send_request(
                                request={"method": "start_feed_assist",
        ...
        for i in range(min(len(slots), 4)):
            ...
            if self.auto_feed_step[i] == 0:
                ...
                if self.port_sensor_hit[i]:
                    ...
                    self.send_request(
                        request={"method": "stop_feed_filament", "params": {"index": i}},
                        callback=None)
                    self.auto_feed_step[i] = 2
                    self.feed_sent[i] = False
    ...
    def _reader(self):
        while self._connected:
            try:
                ...
                logging.warning("ACE: Malformed JSON from device: %s", e)
